...
glimpse(daphnia)
```
We are wondering whether water flea growth rates are impacted by parasites.  From the `glimpse()` function, we can see there are 3 different variables that we can use for figure making. Lets make a boxplot to look at this real quick.
...
Now I will attempt to make the graph that they made in the book:
```{r}
